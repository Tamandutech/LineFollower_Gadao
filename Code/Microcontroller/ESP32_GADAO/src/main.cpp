...
const char *ssid = "VIVAVOX";
const char *password = "vemprafibra";
...
FtpServer ftpSrv; // set #define FTP_DEBUG in ESP8266FtpServer.h to see ftp
                  // verbose on serial
...
void setup() {
  ...
  if (digitalRead(0)) {
    ...
    if (SPIFFS.begin(true)) {
      ftpSrv.begin(
          "braia",
          "guerreiro"); // username, password for ftp.  set ports in
